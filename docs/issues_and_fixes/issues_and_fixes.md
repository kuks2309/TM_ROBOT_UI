...
### [Issue] "그리퍼 통신 끊김" 보고 — 실체는 무이동 명령+래치 상태 오탐 (2차)
- **문제**: 사용자 보고 "그리퍼 통신이 끊어졌다"(15:42). 진단: RTU 링크·어댑터·권한·pyserial 전부 정상. 커널 저널 09:03:44 USB 허브 순간 분리→2초 후 재연결 1건(그 순간 Job 만 영향). 실제 재현 결함: 이미 열린 그리퍼(0.0mm, Dropping 래치)에 open 0.0 재명령 → `(False, '낙하 감지 (pos 0.0mm)')` — 같은 위치 재명령이 매번 실패해 "끊김"처럼 보임.
- **원인**: 목표=현재 위치면 장치가 움직이지 않아 상태 레지스터 미갱신 → 전날 유예(0.3s) 경과 후 래치 Dropping 을 신선한 값으로 오판. "무이동 즉시 성공"이 래치 In place 일 때만 성립하던 사각지대.
