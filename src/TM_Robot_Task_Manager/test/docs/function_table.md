...
갱신: 2026-08-31 (무이동 사각지대 테스트 1종 추가·앵커 재실측. 2026-08-30 래치 유예 2종. 신설 2026-08-29. 기존 test_*.py 의 권위 표는 docs/code_review/TM_Robot_UI-전체/2026-08-29.md 부 4 참조 — 중복 등재하지 않음)
...
| `test_same_position_with_stale_dropping_is_noop_success` | test_sdc_gripper.py:97-103 | 이미 목표 위치+래치 Dropping 잔존 시 무이동 성공(장치 상태 미갱신 사각지대) |
| `test_stale_clamping_on_open_is_ignored` | test_sdc_gripper.py:106-113 | open 시 래치 Clamping 을 '파지 완료'로 오판하지 않음 |
| `test_out_of_range_rejected_without_serial` | test_sdc_gripper.py:116-126 | 위치·속도·전류 범위 밖 → 포트 미개방 거부 |
| `test_timeout_reports_last_state` | test_sdc_gripper.py:129-133 | 타임아웃 실패 + 마지막 상태 보고 |
| `test_write_failure_reports_register` | test_sdc_gripper.py:136-140 | ack 무응답 → 기록 실패 사유 |
