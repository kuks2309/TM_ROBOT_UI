...
- **개정 2(2026-08-30 15:10)**: 마커 자세 내장 — 목표 자세를 실행 시마다 마커 법선 스냅(정렬 offset 재독)으로 계산해 위치와 동시 명령(자기완결, 정렬 생략해도 마커 자세 보장). 계산은 `_marker_perpendicular_orientation` 헬퍼로 추출해 정렬 job 과 공유. 테스트 11건 PASS(자세 기대값·align 항목 부재 거부 추가)·회귀 926 passed. ADR §개정 2.
- **개정(2026-08-30)**: 사용자 요구("기준 옵셋에서 보정을 넣고 싶은 것임")로 dx/dy/dz 보정 파라미터 추가 — 목표 = 마커위치 + R_marker@(YAML 기준 오프셋 + 보정), 기본 0(하위 호환). GUI 옵션 필드 자동 노출. 테스트 10건 PASS(보정 가산 수학 검증 추가)·회귀 916 passed. ADR §개정 참조.
