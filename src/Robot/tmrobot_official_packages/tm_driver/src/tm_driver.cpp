...
TmDriver::TmDriver(const std::string &ip)
	: svr{ ip, 1048576 }  // 1MB buffer for image data
	, sct{ ip, 131072, isOnListenNode }  // 128KB buffer
	, state{ svr.state }
{
}
...
TmDriver::TmDriver(const std::string &ip,
	...
	: svr{ ip, 1048576, psvr_cv }  // 1MB buffer for image data
	, sct{ ip, 131072, isOnListenNode,psct_cv }  // 128KB buffer
	, state{ svr.state }
{
	if (psvr_cv) {
		_svr_cv = psvr_cv;
