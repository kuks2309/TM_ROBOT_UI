...
| 패키지 | 커밋 | 내용 |
|---|---|---|
| `hal/` | `a047d89` (+리뷰 주석 보완 `7ee14cd`) | `zefg_registers.hpp` 레지스터 계약 상수(전 항목 매뉴얼 p5/실측 인용) + `ZefgHal`(RtuClient 위 어댑터 — IEEE754 상위워드 우선 float 변환[실측 0x420C0000=35.0]·RtuError→HalError 고정 매핑·`lastExceptionCode()`[Ruling 7 승인]) + 테스트 7종 + `tools/zefg_hal_h0_smoke.cpp`(읽기 전용 H0 도구, Task 4 계획분 조기 인출) |
...
| `motion/` | `decfc32` (+리뷰 Minor fix `2da382f`) | `ZefgSequencer` FSM(tick 기반·시계 주입·hal 호출 ≤1회/tick·**status_grace 신선도 게이트**) + 테스트 8종(ZefgPlant+RtuClient+ZefgHal 실조립) |
...
## 총검증 실수치 (Task 4 Step 3, 2026-08-30 본 PC 재실행)

| 항목 | 결과 |
|---|---|
| hitbot hal 패키지 ctest | **4/4 PASS** — `hitbot_zefg_hal_test` gtest **7/7** + modbus_rtu 3 엔트리 동거(아래) |
| hitbot sim 패키지 ctest | **1/1 PASS** — `hitbot_zefg_plant_test` gtest **7/7** |
| hitbot motion 패키지 ctest | **1/1 PASS** — `hitbot_zefg_sequencer_test` gtest **8/8** |
